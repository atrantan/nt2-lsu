...
This Section confronts the performances of HPX GMRES implementation with the one provided by PETSC \cite{Petsc}. Recognized as a reference in HPC field, PETSC performs its parallel calculations using MPI. These performance tests have been done for 5 problems from Matrix Market Collection and using the same initial guess $x_{0}$ and right-hand side vector $b$. We fixed the restart parameter and the maximum iteration respectively to 30 and 1000. The used machine was a 2*6 core NUMA node. At this time, HPX doesn't take care in giving NUMA domain affinity to PX-thread. This could be problematic when applications are memory bound: this is the case for GMRES when it uses BLAS interface. We will see below the impact on the performances.  
...
\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.31]{Images/add20.png}
\end{center}
\caption{Performance measures of add20}
\label{add20}
\end{figure}

\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.31]{Images/orsirr1.png}
\end{center}
\caption{Performance measures of orsirr\_1}
\label{orsirr_1}
\end{figure}

\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.31]{Images/orsreg1.png}
\end{center}
\caption{Performance measures of orsreg\_1}
\label{orsreg_1}
\end{figure}

\begin{figure}[h]
\begin{center}
\includegraphics[scale=0.31]{Images/pde900.png}
\end{center}
\caption{Performance measures of pde900}
\label{pde900}
\end{figure}
