...
struct dgetrf_f
{
  ...
  {
    CORE_dgetrf_incpiv(m, n, ib, &A[k*nb + k*nb*LDA], LDA
    , &IPIV[k*nb+k*LDA], &info);
  }
...
int dgetrf(int M, int N, int nb, int ib, double * A, int LDA, double * L, int LDL, int * IPIV) {
    ...
    for(int mm = k+1; mm < TILES; mm++) {
      ...
      Tiles[dst](mm-k,0) = when_all(Tiles[dst](mm-k-1,0),
                           ...
                           .then(dtstrf_f(A,L,m_,kn,nb,ib,k,mm,LDA,LDL,IPIV));
    }
    ...
    for(int nn = k+1; nn < TILES; nn++) {
            for(int mm=k+1; mm < TILES; mm++) {
                ...
                Tiles[dst](mm-k,nn-k) = when_all(  Tiles[dst](mm-k,0),
                                        ...
                                        .then(dssssm_f(A,L,m_,n_,nb,ib,k,mm,nn,LDA,LDL,IPIV));
            }
    }
